...
```{r}
setwd("/Users/lsisk/iCloud_Drive/Documents/Documents_Laura_MacBook/grad_school/dissertation_project/hpg_mice_info/omics_analysis/HPG_puberty_paper/data_upload_for_github")
```
...
group_color_list <- c(Female_Mutant = "#332288", Male_Mutant = "#E6A316", 
                      Female_Wild_Type = "#E066A6", 
                      Male_Wild_Type = "#256769")
...
```
Write metagenomics clr data to csv for use in networks
```{r}
...
feces_map_metabolites$Group <- factor(feces_map_metabolites$Group, levels = c("Female_Mutant", "Male_Mutant", "Female_Wild_Type", "Male_Wild_Type"))

```



# 1 Network stat plots
...
#unname(cohens_d(Prevotellaceae ~ Week, data =  feces_family_top_10_all_samples)$effsize)


effect_bactfamily_sex_list <- as.list(1:num_feces_top_10_family)
names(effect_bactfamily_sex_list) <- feces_top_10_family_names

# loop over names
for(i in feces_top_10_family_names){ 
  tmp_formula_family_effect <- as.formula( paste( 
    i, "~",paste(c("Sex"))))
  effect_bactfamily_sex_list[[i]] <- unname(cohens_d(tmp_formula_family_effect, data =  feces_family_top_10_all_samples)$effsize)
}


effect_bactfamily_Genotype_list <- as.list(1:num_feces_top_10_family)
names(effect_bactfamily_Genotype_list) <- feces_top_10_family_names

# loop over names
for(i in feces_top_10_family_names){ 
  tmp_formula_family_effect <- as.formula( paste( 
    i, "~",paste(c("Genotype"))))
  effect_bactfamily_Genotype_list[[i]] <- unname(cohens_d(tmp_formula_family_effect, data =  feces_family_top_10_all_samples)$effsize)
}


effect_bactfamily_Week_list <- as.list(1:num_feces_top_10_family)
names(effect_bactfamily_Week_list) <- feces_top_10_family_names

# loop over names
for(i in feces_top_10_family_names){ 
  tmp_formula_family_effect <- as.formula( paste( 
    i, "~",paste(c("Week"))))
  effect_bactfamily_Week_list[[i]] <- unname(cohens_d(tmp_formula_family_effect, data =  feces_family_top_10_all_samples)$effsize)
}

effect_bactfamily_sex_genotype_df <- data.frame(cbind(unlist(effect_bactfamily_sex_list), unlist(effect_bactfamily_Genotype_list)))

colnames(effect_bactfamily_sex_genotype_df) <- c("Sex Effect", "Genotype Effect")

write.csv(effect_bactfamily_sex_genotype_df, file = "./results/family_effect_sizes.csv")
```
...
### Genes and orthologs Beta Dispersion NEW SECTION
Weeks combined- Keep
```{r}
# this was done in permanova 
# genes_aitchison_dist_c45<-vegdist(as.matrix(t(genes_tx)) , method='euclidean')

genes_aitchison_dist_betadisp <- betadisper(
  d = genes_aitchison_dist_c45, 
  group = feces_map_metabolites$Group_Week, 
  type = 'centroid')
                                                   
genes_aitchison_dist_betadisp_data <- data.frame(genes_aitchison_dist_betadisp$distances) 

colnames(genes_aitchison_dist_betadisp_data) <- "Distance"
genes_aitchison_dist_betadisp_data <- cbind(genes_aitchison_dist_betadisp_data, feces_map_metabolites)

p_betadisp_feces_genes_week<- ggplot(
  data = genes_aitchison_dist_betadisp_data, 
  aes(x = Group, y = Distance, color = Group)) + 
  geom_boxplot() +
  geom_jitter(width = 0.3) + 
  geom_line(alpha = 0.5, aes(group = Mouse)) + 
  scale_color_manual(values = group_color_list) +
  facet_grid(~Week) +
  theme_NMDS()  +
  #ylim(1000, 3500) +
...
p_betadisp_feces_genes_week

ggsave('./results/p_betadisp_feces_genes_week.png',width=8, height=4, plot = p_betadisp_feces_genes_week) 

genes_aitchison_dist_betadisp_data_ttest <- genes_aitchison_dist_betadisp_data %>%
  group_by(Week, Sex) %>%
  summarise(average = mean(Distance), isnorm = shapiro.test(Distance)$p.value, gendifp = t.test(Distance~Genotype)$p.value)

write.csv(file = "./results/genes_aitchison_dist_betadisp_data_ttest.csv", genes_aitchison_dist_betadisp_data_ttest)
...
orthologs_aitchison_dist_betadisp <- betadisper(
  d = orthologs_aitchison_dist_c45, 
  group = feces_map_metabolites$Group_Week, 
  type = 'centroid')
                                                   
orthologs_aitchison_dist_betadisp_data <- data.frame(orthologs_aitchison_dist_betadisp$distances) 

colnames(orthologs_aitchison_dist_betadisp_data) <- "Distance"
orthologs_aitchison_dist_betadisp_data <- cbind(orthologs_aitchison_dist_betadisp_data, feces_map_metabolites)

p_betadisp_feces_orthologs_week<- ggplot(
  data = orthologs_aitchison_dist_betadisp_data, 
  aes(x = Group, y = Distance, color = Group)) + 
  geom_boxplot() +
  #ylim(1000, 3500) +
  geom_jitter(width = 0.3) + 
...
ggsave('./results/p_betadisp_feces_orthologs_week.png',width=8, height=4, plot = p_betadisp_feces_orthologs_week) 


#with lines

orthologs_aitchison_dist_betadisp_data$Week_Sex <- paste0(orthologs_aitchison_dist_betadisp_data$Week, orthologs_aitchison_dist_betadisp_data$Sex)

orthologs_aitchison_dist_betadisp_data$Week_Sex <- factor(orthologs_aitchison_dist_betadisp_data$Week_Sex , levels = c("3Female", "10Female", "3Male", "10Male"))

#dont use too messy
p_betadisp_orthologs_week_group_lines<- ggplot(
  data = orthologs_aitchison_dist_betadisp_data, 
  aes(x = Week_Sex, y = Distance, color = Group)) + 
  geom_boxplot() +
  geom_point() + 
  geom_line(alpha = 0.5, aes(group = Mouse)) + 
  scale_color_manual(values = group_color_list) +
  ...
  theme_NMDS()  +
  theme(axis.title.x=element_blank()) # ,
        #axis.text.x=c("3", "3", "10", "10"))#,
        #axis.ticks.x=element_blank())
p_betadisp_orthologs_week_group_lines
ggsave('./results/p_betadisp_orthologs_week_group_lines.png',width=8, height=5, plot = p_betadisp_orthologs_week_group_lines) 
```
...
# Genes- differential functions
Sum counts by module and then clr
```{r}
gene_read_counts_and_metadata[is.na(gene_read_counts_and_metadata)] <- 0
#module
gene_read_counts <- gene_read_counts_and_metadata[,1:74]
gene_kegg_module <- gene_read_counts_and_metadata$KEGG_Module
gene_read_counts_and_kegg_module <- cbind(gene_read_counts, gene_kegg_module)

kegg_module_read_counts <- gene_read_counts_and_kegg_module %>% group_by(gene_kegg_module) %>% 
  summarise(across(everything(), sum)) %>%
  as.data.frame()

dim(kegg_module_read_counts)
```

Select just the count data 
```{r}
row.names(kegg_module_read_counts) <-kegg_module_read_counts$gene_kegg_module
head(kegg_module_read_counts[,2:75])
kegg_module_untx <- kegg_module_read_counts[3:960,2:75]
kegg_module_untx[1:10, 1:5]

#clr
kegg_module_zerorep <- kegg_module_untx
kegg_module_zerorep[kegg_module_zerorep == 0] <- 0.001
kegg_module_tx<- data.frame(t(clr(t(kegg_module_zerorep))))
dim(kegg_module_tx)

kegg_module_tx[1:10, 1:5]
```

Test whether significant lipid related modules are significantly different
```{r}
#phosphatidylethanolamine modules
#kegg_module_tx_phospha <- kegg_module_tx[grep("M00092|M00093", rownames(kegg_module_tx)), ]

# FA modules
#kegg_module_tx_FA<- kegg_module_tx[grep("M00082|M00083|M00085|M00415|M00873|M00874|RM02001|RM02002|RM02101|RM02102", rownames(kegg_module_tx)), ]

# combine
#kegg_module_tx_siglipids <- rbind(kegg_module_tx_phospha, kegg_module_tx_FA)

siglipid_modules <- c("M00092", "M00093", "M00082", "M00083", "M00085", "M00415", "M00873", "M00874", "RM02001", "RM02002", "RM02101", "RM02102")

kegg_module_tx_siglipids <- filter(kegg_module_tx, row.names(kegg_module_tx) %in% siglipid_modules)


#transpose
kegg_module_tx_siglipids_t <- data.frame(t(kegg_module_tx_siglipids))

#change the number of names to how many modules there are
feces_siglipid_module_names <- colnames(kegg_module_tx_siglipids_t)
num_feces_siglipid_module <- length(feces_siglipid_module_names)

# create a named list to hold the fitted models
feces_lipid_module_model_list <- as.list(1:num_feces_siglipid_module)
names(feces_lipid_module_model_list) <- feces_siglipid_module_names

feces_lipid_module_model_anova_list <- as.list(1:num_feces_siglipid_module)
names(feces_lipid_module_model_anova_list) <- feces_siglipid_module_names


kegg_module_tx_siglipids_meta <- cbind(kegg_module_tx_siglipids_t, feces_map_metabolites)

# loop over names
for(i in feces_siglipid_module_names){ 
  # create temporary data matrix and model formula
  tmp_sig <- kegg_module_tx_siglipids_meta[ , c(i,"Sex","Genotype","Week", "Mouse")]
  fml_sig <- as.formula( paste( i, "~", paste(c("Sex","Genotype", "Week"), collapse="*") ) )
  # assign fit to list by name
  feces_lipid_module_model_list[[i]] <- lme(fml_sig, random = list( ~1|Mouse), method = "ML", data= tmp_sig)
  feces_lipid_module_model_anova_list[[i]] <- Anova(lme(fml_sig, random = list(~1|Mouse), method = "ML", data= tmp_sig))
}

capture.output(feces_lipid_module_model_anova_list, file="./results/lme_feces_siglipid_module_anova.csv")

capture.output(feces_lipid_module_model_list, file="./results/lme_feces_siglipid_module_models.csv")

#and FDR correction
p_values_feces_siglipid_anova_table <- data.frame(lapply(feces_lipid_module_model_anova_list, "[", , "Pr(>Chisq)"))
rownames(p_values_feces_siglipid_anova_table) <- rownames(feces_lipid_module_model_anova_list$M00092)

#write.csv(p_values_feces_siglipid_anova_table, file="./results/feces_analysis/16S/taxa_barplots/family_abundance_stats/lme_feces_siglipid_anova_p_value_table.csv")

p_values_feces_siglipid_anova_table_fdr <- t(data.frame(apply(p_values_feces_siglipid_anova_table, 1, function(x) p.adjust(x, method = "fdr"))))

write.csv(p_values_feces_siglipid_anova_table_fdr, file="./results/lme_feces_siglipid_anova_p_value_table_FDR.csv")
...
Graph
```{r}
for (col_name in colnames(kegg_module_tx_siglipids_t[1:5])) { 
  # Create ggplot object
  p <- ggplot(data = kegg_module_tx_siglipids_meta , aes(x = Group, y = .data[[col_name]], color = Group)) +
    geom_boxplot() +
    geom_point() + 
    scale_color_manual(values = group_color_list) +
    theme_classic() +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 0.5))  +
    facet_wrap(~Week)
    labs(y = paste("CLR-transformed Module Gene Counts", col_name))
    filepathA = paste("./results/siglipid_module_", col_name) 
    
  # Save the graph with ggsave
  ggsave(filename = paste(filepathA, ".png"), plot = p, width = 10, height = 6)
}
```
...
### Metagenomes_binned Species beta dispersion 
Weeks combined- Keep
```{r}
# this was done in permanova 
# meta_binned_species_aitchison_dist_c45<-vegdist(as.matrix(t(meta_binned_species_tx)) , method='euclidean')

meta_binned_species_aitchison_dist_betadisp <- betadisper(
  d = meta_binned_species_aitchison_dist_c45, 
  group = feces_map_metabolites$Group_Week , 
  type = 'centroid') 
                                                   
meta_binned_species_aitchison_dist_betadisp_data <- data.frame(meta_binned_species_aitchison_dist_betadisp$distances)

colnames(meta_binned_species_aitchison_dist_betadisp_data) <- "Distance"
meta_binned_species_aitchison_dist_betadisp_data <- cbind(meta_binned_species_aitchison_dist_betadisp_data, feces_map_metabolites)

p_betadisp_feces_meta_species_week<- ggplot(
  data = meta_binned_species_aitchison_dist_betadisp_data, 
  aes(x = Group, y = Distance, color = Group)) + 
  geom_boxplot() +
  geom_jitter(width = 0.3) + 
  scale_color_manual(values = group_color_list) +
...
ggsave('./results/p_betadisp_feces_meta_species_week.png',width=7, height=4, plot = p_betadisp_feces_meta_species_week) 

# stats- non normal
#Mutants 
meta_binned_species_betadisp_week3_mutant_testp <- wilcox.test(Distance~Sex, data = meta_binned_species_aitchison_dist_betadisp_data[meta_binned_species_aitchison_dist_betadisp_data$Week=="3" & meta_binned_species_aitchison_dist_betadisp_data$Genotype == "Mutant",  ])$p.value

meta_binned_species_betadisp_week10_mutant_testp <- wilcox.test(Distance~Sex, data = meta_binned_species_aitchison_dist_betadisp_data[meta_binned_species_aitchison_dist_betadisp_data$Week=="10" & meta_binned_species_aitchison_dist_betadisp_data$Genotype == "Mutant",  ])$p.value

#Wild_Types 
meta_binned_species_betadisp_week3_Wild_Type_testp <- wilcox.test(Distance~Sex, data = meta_binned_species_aitchison_dist_betadisp_data[meta_binned_species_aitchison_dist_betadisp_data$Week=="3" & meta_binned_species_aitchison_dist_betadisp_data$Genotype == "Wild_Type",  ])$p.value

meta_binned_species_betadisp_week10_Wild_Type_testp <- wilcox.test(Distance~Sex, data = meta_binned_species_aitchison_dist_betadisp_data[meta_binned_species_aitchison_dist_betadisp_data$Week=="10" & meta_binned_species_aitchison_dist_betadisp_data$Genotype == "Wild_Type",  ])$p.value

meta_binned_species_betadisp_testp <- c("week3mutant" = meta_binned_species_betadisp_week3_mutant_testp, "week10mutant" = meta_binned_species_betadisp_week10_mutant_testp, "week3wt" = meta_binned_species_betadisp_week3_Wild_Type_testp, "week10wt" = meta_binned_species_betadisp_week10_Wild_Type_testp)

write.csv(file = "./results/meta_binned_species_betadisp_wilcoxtestp.csv", meta_binned_species_betadisp_testp)
```
...
write.csv(file = "./results/top_amines_sex_diff_week_10_peak_heights.csv", top_5_amines_sex_diff_df_t_week10_t_test_results_andFDR)
```
...
###Effect sizes
```{r}
#Read in what saved earlier
feces_lipids_class_tx_t <- data.frame(t(feces_lipids_class_tx))

important_lipids <- feces_lipids_class_tx_t[, c("Acylcarnitines",
                                                "Fatty_acids", 
                            "N.acyl.lysophosphatidylethanolamines",
                            "Phosphatidylethanolamine_alkenyls", 
                            "Phosphatidylethanolamines")]

feces_amines_class_tx_t <- data.frame(t(feces_amines_class_tx))

important_amines <- feces_amines_class_tx_t[, c("Alkanes",
                                                "Amino_sugars", 
                            "Carboxylic_acids",
                            "Disaccharides", 
                            "Flavonoids", 
...
```{r}
#calculate effect sizes by sex for for each genotype at week 10

important_lipids$Sex <- feces_map_metabolites_week10$Sex
important_lipids$Genotype <- feces_map_metabolites_week10$Genotype

important_amines$Sex <- feces_map_metabolites_week10$Sex
important_amines$Genotype <- feces_map_metabolites_week10$Genotype

# Lipids
effect_lipids_Wild_Type_list <- as.list(1:5)
names(effect_lipids_Wild_Type_list) <- colnames(important_lipids)[1:5]

# loop over names
for(i in colnames(important_lipids)[1:5]){ 
  tmp_formula_meta_effect <- as.formula( paste( 
    i, "~",paste(c("Sex"))))
  effect_lipids_Wild_Type_list[[i]] <- unname(cohens_d(tmp_formula_meta_effect, 
  data =important_lipids[important_lipids$Genotype=="Wild_Type", ])$effsize)
}


effect_lipids_Mutant_list <- as.list(1:5)
names(effect_lipids_Mutant_list) <- colnames(important_lipids)[1:5]

# loop over names
for(i in colnames(important_lipids)[1:5]){ 
  tmp_formula_meta_effect <- as.formula( paste( 
    i, "~",paste(c("Sex"))))
  effect_lipids_Mutant_list[[i]] <- unname(cohens_d(tmp_formula_meta_effect, 
  data =important_lipids[important_lipids$Genotype=="Mutant", ])$effsize)
}



effect_lipids_sex_df <- data.frame(cbind(unlist(effect_lipids_Wild_Type_list ), unlist(effect_lipids_Mutant_list )))

colnames(effect_lipids_sex_df ) <- c("Wild_Type", "Mutant")

write.csv(effect_lipids_sex_df , file = "./results/lipids_sex_effect_sizes.csv")

# Amines
effect_amines_Wild_Type_list <- as.list(1:10)
names(effect_amines_Wild_Type_list) <- colnames(important_amines)[1:10]

# loop over names
for(i in colnames(important_amines)[1:10]){ 
  tmp_formula_meta_effect <- as.formula( paste( 
    i, "~",paste(c("Sex"))))
  effect_amines_Wild_Type_list[[i]] <- unname(cohens_d(tmp_formula_meta_effect, 
  data =important_amines[important_amines$Genotype=="Wild_Type", ])$effsize)
}


effect_amines_Mutant_list <- as.list(1:10)
names(effect_amines_Mutant_list) <- colnames(important_amines)[1:10]

# loop over names
for(i in colnames(important_amines)[1:10]){ 
  tmp_formula_meta_effect <- as.formula( paste( 
    i, "~",paste(c("Sex"))))
  effect_amines_Mutant_list[[i]] <- unname(cohens_d(tmp_formula_meta_effect, 
  data =important_amines[important_amines$Genotype=="Mutant", ])$effsize)
}



effect_amines_sex_df <- data.frame(cbind(unlist(effect_amines_Wild_Type_list ), unlist(effect_amines_Mutant_list )))

colnames(effect_amines_sex_df ) <- c("Wild_Type", "Mutant")

write.csv(effect_amines_sex_df , file = "./results/amines_sex_effect_sizes.csv")
```
